    ...
    <div class="content">
      ...
      <div class="new-user" v-for="product in xinShouBao" :key="product.id">
        <div class="new-user-sm">
          ...
          <span>投资最高限额{{product.bitMaxLimit}}元</span>
          <span>当日即系</span>
        </div>
      ...
      <ul class="preferred-select clearfix">
        <li v-for="(product,index) in youXuan" :key="product.id">
          <h3 class="preferred-select-title">
            ...
            <img src="@/assets/image/1-bg2.jpg" alt="" v-if="index == 1">
            <img src="@/assets/image/1-bg3.jpg" alt="" v-if="index == 2">
          </h3>
...
export default {
  ...
  mounted() {
    // when page is loaded to browser, this method will be executed
    ...
    // fetch product data
    doGet("/v1/product/index").then(response => {
      if (response.data.code === 1000) {
        this.xinShouBao = response.data.data.xinShouBao;
        this.youXuan = response.data.data.youXuan;
        this.sanBiao = response.data.data.sanBiao;
      }
    });
